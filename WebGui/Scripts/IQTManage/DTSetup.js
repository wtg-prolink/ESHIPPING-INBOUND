...
function initQTGrid() {
    ...
    function getcityop(name) {
        ...
        var city_op = getLookupOp("MainGrid",
            {
                url: rootPath + 'TPVCommon/GetBsdistforLookup',
                config: LookUpConfig.BsdistLookup,
                returnFn: function (map, $grid) {
                    var selRowId = $grid.jqGrid('getGridParam', 'selrow');
                    if (_name === "PolCd")
                        setGridVal($grid, selRowId, "PolNm", map.DistNm, null);
                    else {
                        setGridVal($grid, selRowId, "PodNm", map.DistNm, null);
                        setGridVal($grid, selRowId, "State", map.StateCd, null);
                    }
                    setGridVal($grid, selRowId, _name, map.DistCd, "lookup");
                    return map.DistCd;
                }
            }, LookUpConfig.BsdistCdAuto(groupId, $grid,
            function ($grid, rd, elem, rowid) {
                if (_name === "PolCd")
                    setGridVal($grid, rowid, "PolNm", rd.DIST_NM, null);
                else {
                    setGridVal($grid, rowid, "PodNm", rd.DIST_NM, null);
                    setGridVal($grid, rowid, "State", rd.STATE_CD, null);
                }
                //$(elem).val(rd.CNTRY_CD + rd.PORT_CD);
                setGridVal($grid, rowid, _name, rd.DIST_CD, "lookup");
            }), { param: "" });
        return city_op;
    }
    ...
    var quotTypeCols = _quotType.split(";");
    $.each(quotTypeCols, function (index, val) {
        var colNames = quotTypeCols[index].split(":");
        if (colNames.length > 1) {
            var colName = colNames[0], colTitle = colNames[1];
            switch (colName) {
                case "F10":
                case "F1":
                case "F2":
                ...
                case "F9":
                    break;
                default:
                    colModel.push({ name: colName, title: colTitle + ' ' + '@Resources.Locale.L_DTQuery_Funit', index: colName, width: 80, align: 'right', formatter: 'number', formatoptions: { decimalSeparator: ".", thousandsSeparator: ",", decimalPlaces: 2, defaultValue: '0.00' }, hidden: false, editable: true });
                    break;
            }
        }
    });
    colModel.push({ name: 'Remark', title: '@Resources.Locale.L_BSCSSetup_Remark', index: 'Remark', width: 150, align: 'left', sorttype: 'string', hidden: false, editable: true, editoptions: { size: 500, maxlength: 500 } });
    colModel.push({ name: 'SeqNo', title: '@Resources.Locale.L_NRSSetup_SeqNo', index: 'SeqNo', sorttype: 'string', width: 250, hidden: true, editable: false });
    colModel.push({ name: 'QuotNo', title: '@Resources.Locale.L_QTQuery_QuotNo', index: 'QuotNo', sorttype: 'string', width: 100, editable: false, hidden: true });
    ...
    function getcur(name) {
        ...
        var cur_op = getLookupOp("MainGrid",
            ...
            }, LookUpConfig.GetCurAuto(groupId,undefined, $grid,
            function ($grid, rd, elem, rowid) {
                $(elem).val(rd.CUR);
   ...
   var colModel2 = [
        ...
        { name: 'Repay', title: '@Resources.Locale.L_SMCHGSetup_ChgRepay', index: 'Repay', sorttype: 'string', width: 100, hidden: false, editable: false },
    ...
    */
    ...
    _handler.intiGrid("SubGrid2", $grid2, {
        ...
        beforeAddRowFunc: function (rowid) {
            ...
            {
                VenderCd = MainData[0]["LspCd"];
            }
            $.ajax({
                async: true,
                url: rootPath + "IQTManage/GetTransTypeInfo",
                type: 'POST',
                data: {VenderCd: VenderCd},
                async:false,
            ...
            var colModel = [
                ...
                { name: 'TranType', title: '@Resources.Locale.L_UserSetUp_TranType', index: 'TranType', editable: true, width: 80, align: 'left', sorttype: 'string', hidden: false, formatter: "select", editoptions: { value: select_trantype }, edittype: 'select' },
                { name: 'PolCd', title: '@Resources.Locale.L_BaseLookup_PolCd', index: 'PolCd', sorttype: 'string', edittype: 'custom', editoptions: gridLookup(getcityop("PolCd")), width: 80, hidden: false, editable: true },
                ...
                { name: 'PodCd', title: '@Resources.Locale.L_BaseLookup_PodCd', index: 'PodCd', sorttype: 'string', edittype: 'custom', editoptions: gridLookup(getcityop("PodCd")), width: 80, hidden: false, editable: true },
                ...
                { name: 'Tt', title: '@Resources.Locale.L_RouteSetup_Tt', index: 'Tt', width: 60, align: 'right', formatter: 'integer', hidden: false, editable: true },
                    ...
                    if(result.message == "success")
                    {
                        ...
                        $.each(transTypeCols, function (index, val) {
                            colModel.push({ name: transTypeColsN[index], title: val, index: transTypeColsN[index], sorttype: 'float', width: 100, align: 'right', formatter: 'number', formatoptions: { decimalSeparator: ".", thousandsSeparator: ",", decimalPlaces: 2, defaultValue: '0.00' }, editable:true, hidden: false });
                            //colNames.push(val);
                        });
                    }
                    
                    colModel.push({ name: 'UId', title: 'UId', index: 'UId', sorttype: 'string', align: 'left', width: 120, hidden: true });
                    colModel.push({ name: 'Remark', title: '@Resources.Locale.L_BSCSSetup_Remark', index: 'Remark', sorttype: 'string', width: 200, hidden: false, editable: true, editable: true, editoptions: { size: 500, maxlength: 500 } });
                    _handler.intiGrid("MainGrid", $grid, {
                        multiselect: true,
                        colModel: colModel, caption: jslang["L_QTManage_List"], delKey: ["UId"],
                        onAddRowFunc: function (rowid) {
                            var maxSeqNo = $grid.jqGrid("getCol", "SeqNo", false, "max");
                            if (typeof maxSeqNo === "undefined")
                                maxSeqNo = 0;
