...
jQuery(document).ready(function ($) {
    ...
    var colModel = [
            ...
            { name: 'DnType', title: '@Resources.Locale.L_PostBillSetup_DnType', index: 'DnType', editoptions: gridLookup(getop("DnType")), edittype: 'custom', sorttype: 'string', width: 100, hidden: false, editable: true },
            { name: 'Bu', title: '@Resources.Locale.L_PostBillSetup_Bu', index: 'Bu', editoptions: gridLookup(getop("Bu")), edittype: 'custom', sorttype: 'string', width: 100, hidden: false, editable: true },
            { name: 'ChannelCd', title: '@Resources.Locale.L_PostBillSetup_ChannelCd', index: 'ChannelCd', editoptions: gridLookup(getop("ChannelCd")), edittype: 'custom', sorttype: 'string', width: 100, hidden: false, editable: true },
            { name: 'CentDecl', title: '@Resources.Locale.L_DNApproveManage_CentDecl', index: 'CentDecl', width: 100, sorttype: 'string', editable: true, formatter: "select", edittype: 'select', editoptions: { value: 'N:N;Y:Y', defaultValue: 'N', dataInit: function (elem) { $(elem).addClass('form-control input-sm'); } } },
            { name: 'DeepProcess', title: '@Resources.Locale.L_DNApproveManage_DeepProcess', index: 'DeepProcess', width: 100, sorttype: 'string', editable: true, formatter: "select", edittype: 'select', editoptions: { value: 'N:N;Y:Y', defaultValue: 'N', dataInit: function (elem) { $(elem).addClass('form-control input-sm'); } } },
            { name: 'BandType', title: '@Resources.Locale.L_PostBillSetup_BandType', index: 'BandType', width: 100, sorttype: 'string', editable: true, formatter: "select", edittype: 'select', editoptions: { value: 'N:N;Y:Y', defaultValue: 'N', dataInit: function (elem) { $(elem).addClass('form-control input-sm'); } } },
            ...
            { name: 'PartyFctype', title: '@Resources.Locale.L_PostBillSetup_PartyFctype', index: 'PartyFctype', editoptions: gridLookup(getpartyop()), edittype: 'custom', sorttype: 'string', width: 140, hidden: false, editable: true },
            { name: 'IsOutdate', title: '@Resources.Locale.L_PostBillSetup_IsOutdate', index: 'IsOutdate', width: 100, sorttype: 'string', editable: true, formatter: "select", edittype: 'select', editoptions: { value: 'N:N;Y:Y', defaultValue: 'N', dataInit: function (elem) { $(elem).addClass('form-control input-sm'); } } },
            { name: 'IsBroker', title: '@Resources.Locale.L_PostBillSetup_IsBroker', index: 'IsBroker', width: 100, sorttype: 'string', editable: true, formatter: "select", edittype: 'select', editoptions: { value: 'N:N;Y:Y', defaultValue: 'N', dataInit: function (elem) { $(elem).addClass('form-control input-sm'); } } },
            { name: 'IsAta', title: 'ATA', index: 'IsAta', sorttype: 'string', width: 100, editable: true, formatter: "select", edittype: 'select', editoptions: { value: 'N:N;Y:Y', defaultValue: 'N', dataInit: function (elem) { $(elem).addClass('form-control input-sm'); } } }
    ];
    _handler.saveUrl = rootPath + "ACTRANTYPE/PostBillUpdate";
    _handler.inquiryUrl = rootPath + "ACTRANTYPE/PostBillQuery";
    ...
    function getop(name) {
        ...
        }else if(_name=="ChannelCd")
        {
            ...
            lookupurl = LookUpConfig.TermUrl;
            lookupcode = 'TD';
        }
    ...
    function getpartyop(name) {
        var _name = name;
        var city_op = getLookupOp("PostBillGrid",
            {
                url: rootPath + LookUpConfig.PartyNoUrl,
                config: LookUpConfig.SmptyLookup,
                returnFn: function (returnObj, $grid) {
                    var selRowId = $grid.jqGrid('getGridParam', 'selrow');
                    setGridVal($grid, selRowId, 'PartyFctype', returnObj.PartyNo, 'lookup');
                    return returnObj.PartyNo;
                }
            }, {
                baseConditionFunc: function () {
                    var PartyType = "FC";
                    return encodeURIComponent("PARTY_TYPE LIKE '%" + PartyType + ";%'");
                }
            }, LookUpConfig.GetPartyNoAuto(groupId, $PostBillGrid,
            function ($grid, rd, elem, rowid) {
                var selRowId = rowid;
                setGridVal($grid, selRowId, 'PartyFctype', rd.PARTY_NO, 'lookup');
            }));
        city_op.param = "";
        return city_op;
    }
