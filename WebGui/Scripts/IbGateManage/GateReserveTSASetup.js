...
$(function () {
    var colModel1 = [
        ...
        {
            name: 'Qty', title: 'QTY', index: 'Qty', width: 70, align: 'right', sorttype: 'float', editable: true,
            formatter: 'number',
            formatoptions: {
                decimalSeparator: ".",
                thousandsSeparator: ",",
        ...
        { name: 'SealNo2', title: 'Seal No2', index: 'SealNo2', width: 120, align: 'left', sorttype: 'string', hidden: false, editable: false },
            {
                name: 'Gw', title: 'GW', index: 'Gw', width: 70, align: 'right', sorttype: 'float', editable: true,
                formatter: 'number',
                formatoptions: {
                    decimalSeparator: ".",
                    thousandsSeparator: ",",
...
function initLoadData(Uid) {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            MenuBarFuncArr.Enabled(["MBAddConatiner", "MBRemoveConatiner","MBReserveConfirm", "MBDel", "MBEdit", "MBCopy", "MBEdoc", "MBApprove", "MBCHM", "MBPomc", "MBPrint", "MBMoney", "TSAVoid", "TSARestore"]);
...
function _initMenu() {
    ...
    if ($("#Status").val())
        MenuBarFuncArr.MBCancel = function () {
            var Status = $("#Status").val();
            MenuBarFuncArr.Enabled(["MBReserveConfirm", "MBModifyConfirm", "MBWHDelivery", "MBAddConatiner","MBRemoveConatiner","TSARestore","TSAVoid"]);
            chkMenu(Status);
            _status = "";
        }

    MenuBarFuncArr.AddMenu("MBReserveConfirm", "glyphicon glyphicon-ok", "Make Appointment", function () {
        var ReverseNo = $("#ReverseNo").val();
        var Status = $("#Status").val();

        var count=parseInt($("#CntrCount").val());
        
        if (count > 0) {
            if (Status != "T" && Status != "W") {
                CommonFunc.Notify("", "The Make Appointment can be only done with the status of Transloading or Direct Delivery!", 1000, "warning");
                return false;
            }
        }
        ...
        var reservedate = $("#ReserveDate").val();
        if (reservedate === undefined || reservedate === "" || reservedate == null) {
            alert("Delivery Date is Null!");
            return false;
        }
        ...
        if (ReverseNo != "") {
            ...
            $.ajax({
                async: true,
                url: rootPath + "IbGateManage/TSAMakeAppoint",
                type: 'POST',
                data: { UId: $("#UId").val() },
                dataType: "json",
                beforeSend: function () {
                ...
                success: function (result) {
                    ...
                    if (result.message == "success") {
                        CommonFunc.Notify("", "Make Appoint Successful", 1000, "success");
                        initLoadData(_uid);
                    }
                    else {
    ...
    MenuBarFuncArr.AddMenu("TSAVoid", "glyphicon glyphicon-bell", _getLang("Void", "Void"), function () {
        var ReverseNo = $("#ReverseNo").val();
        var Status = $("#Status").val();

        if (Status != "T" && Status != "R" && Status != "W") {
            CommonFunc.Notify("", _getLang("Only Transloading or Slottime booked,Direct Delivery status can do this process!"), 1000, "warning");
            return false;
        }

        var _confirm = confirm("Are you sure to void it？");
        if (!_confirm) {
            return;
        }
        ...
        if (ReverseNo != "") {
            $.ajax({
                async: true,
                url: rootPath + "IbGateManage/SetStatus",
                type: 'POST',
                data: { UId: $("#UId").val(),type:"void" },
                dataType: "json",
                beforeSend: function () {
                    CommonFunc.ToogleLoading(true);
                },
                ...
                success: function (result) {
                    CommonFunc.ToogleLoading(false);
                    if (result.message == "success") {
                        CommonFunc.Notify("","Void success", 1000, "success");
                        initLoadData(_uid);
                    }
                    else {
                        CommonFunc.Notify("", result.message, 1000, "warning");
    ...
    MenuBarFuncArr.AddMenu("TSARestore", "glyphicon glyphicon-bell", _getLang("Restore", "Restore"), function () {
        var ReverseNo = $("#ReverseNo").val();
        var Status = $("#Status").val();

        if (Status != "K" ) {
            CommonFunc.Notify("", _getLang("Only Void status can do this process!"), 1000, "warning");
            return false;
        }

        var _confirm = confirm("Are you sure to restore this data?");
        if (!_confirm) {
            return;
        }
        ...
        if (ReverseNo != "") {
            $.ajax({
                ...
                success: function (result) {
                    CommonFunc.ToogleLoading(false);
                    if (result.message == "success") {
                        CommonFunc.Notify("", "Restore success", 1000, "success");
                        initLoadData(_uid);
                    }
                    else {
                        CommonFunc.Notify("", result.message, 1000, "warning");
    ...
    MenuBarFuncArr.MBSave = function (dtd) {
        ...
        if ($.isEmptyObject(changeData)) {
            ...
            MenuBarFuncArr.Enabled([ "MBReserveConfirm"]);
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                CommonFunc.Notify("", _getLang("L_MailFormatSetup_SaveS", "保存成功"), 500, "success");
                MenuBarFuncArr.SaveResult = true;
                MenuBarFuncArr.Enabled(["MBReserveConfirm", "MBModifyConfirm", "MBWHDelivery", "MBAddConatiner","MBRemoveConatiner","TSAVoid"]);
                _status = "edit";
                chkMenu($("#Status").val());
                dtd.resolve();
                initLoadData(_uid);
...
function caluSum() {
    var $FclMainGrid = $("#FclMainGrid");
    var TtlPqty = $FclMainGrid.jqGrid("getCol", "Qty", false, "sum");
    var Gw = $FclMainGrid.jqGrid("getCol", "Gw", false, "sum");
    var Cbm = $FclMainGrid.jqGrid("getCol", "Cbm", false, "sum");
    $("#PalletQty").val(CommonFunc.formatFloat(TtlPqty, 0));
    $("#Gw").val(CommonFunc.formatFloat(Gw, 4));
    $("#Cbm").val(CommonFunc.formatFloat(Cbm, 4));
}
...
function chkMenu(status) {
    if (status == "O") {
        MenuBarFuncArr.Disabled(["MBEdit", "MBReserveConfirm", "MBModifyConfirm", "MBWHDelivery", "MBAddConatiner", "MBRemoveConatiner","TSAVoid", "TSARestore"]);
    }
    if (status == "V") {
        MenuBarFuncArr.Disabled(["MBEdit", "MBReserveConfirm", "MBModifyConfirm", "MBWHDelivery", "MBAddConatiner","MBRemoveConatiner", "TSAVoid"]);
    }
}
