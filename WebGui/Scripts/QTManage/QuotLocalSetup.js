...
function initQTGrid() {
    ...
    var colModel = [
        ...
        { name: 'PodCd', title: 'POD', index: 'PodCd', editoptions: gridLookup(getcityop("PodCd")), edittype: 'custom', sorttype: 'string', width: 100, hidden: false, editable: true },
    ...
    _handler.intiGrid("MainGrid", $MainGrid, {
        colModel: colModel, caption: '@Resources.Locale.L_QuotLocalSetup_Scripts_local314', delKey: ["UId"], height: 300,
...
$(function () {
    ...
    _handler.beforLoadView = function () {
        ...
        var requires = ["QuotNo", "EffectFrom", "EffectTo", "Rlocation",  "TranType", "PolCd", "LspCd", "TranType"];
        var readonlys = ["QuotDate", "RlocationNm", "LspNm", "QuotType"];

        readonlys = ["QuotDate", "RlocationNm", "LspNm", "QuotType", "LspCd", "CustNm"];
    ...
    _handler.addData = function () {
        //初始化新增数据
        var data = { "TranMode": "X", "TranType": "A", QuotType: "P", "Period": "B", "OutIn": "O", "FreightTerm": "P", LspCd: cmp, Rlocation: cmp, "EffectFrom": getDate(0, "-"), "EffectTo": getDate(365, "-") };
        if (ioflag == "O") data = { "TranMode": "X", "TranType": "A", QuotType: "P", "Period": "B", "OutIn": "O", "FreightTerm": "P", LspCd: cmp,"EffectFrom": getDate(0, "-"), "EffectTo": getDate(365, "-") };
    ...
    _handler.beforSave = function () {
        ...
        nullCols.push({ name: "Repay", index: 15, text: '@Resources.Locale.L_QTSetup_chgCd' });
        nullCols.push({ name: "Punit", index: 16, text: '@Resources.Locale.L_AirQuery_Punit' });
        //nullCols.push({ name: "Cur", index: 16, text: '@Resources.Locale.L_IpPart_Crncy' });
    ...
    function checkDoubleData($grid) {
        ...
        for (var i = 0; i < rowIds.length; i++) {
            ...
            var key = rowDatas["Punit"] + "#" + rowDatas["ChgCd"] + "#" + rowDatas["ChgType"] + "#" + rowDatas["Carrier"] + "#" + rowDatas["PodCd"];
            for (var x = 0; x < checkKey.length; x++) {
                if (key === checkKey[x]) {
                    ...
                    CommonFunc.Notify("", "@Resources.Locale.L_QuotBrokerSetup_Script_174", 2000, "warning");
                    return false;
                }
            }
            for (var j = 0; j < rowIds.length; j++) {
                ...
                var key1 = rowDatas1["Punit"] + "#" + rowDatas1["ChgCd"] + "#" + rowDatas1["ChgType"] + "#" + rowDatas1["Carrier"] + "#" + rowDatas1["PodCd"];
                if (key1 === key) {
                    try {
    ...
    registBtnLookup($("#CreditToLookup"), {
        item: '#CreditTo', url: rootPath + LookUpConfig.PartyNoUrl, config: LookUpConfig.PartyNoLookup, param: "", selectRowFn: function (map) {
            ...
            $("#CreditTo1").val(map.PartyNo);
            $("#CreditNm1").val(map.PartyName);
        }
    }, undefined, LookUpConfig.GetPartyNoAuto(groupId, undefined, function ($grid, rd, elem) {
        $("#CreditTo").val(rd.PARTY_NO);
        $("#CreditNm").val(rd.PARTY_NAME);
        $("#CreditTo1").val(rd.PARTY_NO);
        $("#CreditNm1").val(rd.PARTY_NAME);
    }));

    registBtnLookup($("#CreditToLookup1"), {
        item: '#CreditTo1', url: rootPath + LookUpConfig.PartyNoUrl, config: LookUpConfig.PartyNoLookup, param: "", selectRowFn: function (map) {
            $("#CreditTo").val(map.PartyNo);
            $("#CreditNm").val(map.PartyName);
            $("#CreditTo1").val(map.PartyNo);
            $("#CreditNm1").val(map.PartyName);
