...
$(function () {
    ...
    _handler.beforDel = function () {
        ...
        if (_handler.topData["Status"] === "T") {
            CommonFunc.Notify("", _getLang("L_QTSetup_Terminated", ""), 500, "warning");
            return false;
        }
    }
    ...
    function setDisabled() {
        ...
        $("#FreightTermSelect").siblings("div").children("button").removeClass("disabled");
        $("#FreightTermSelect").siblings("div").find("li").removeClass("disabled");

        $("#OutInSelect").siblings("div").children("button").removeClass("disabled");
        $("#OutInSelect").siblings("div").find("li").removeClass("disabled");
            ...
            for (var i = 0; i < changeData["mt"].length; i++) {
                ...
                if (changeData["mt"][i]["FreightTerm"] != undefined) {
                    if ("F" == tranMode) {
                        ...
                        changeData["mt"][i]["FreightTerm"] = $("#FreightTerm").val();
                    }
                }
                if (changeData["mt"][i]["OutIn"] != undefined) {
                    if ("F" == tranMode) {
                        ...
                        changeData["mt"][i]["OutIn"] = $("#OutIn").val();
                    }
                }
                if (changeData["mt"][i]["Incoterm"] != undefined) {
                    if ("F" == tranMode)
                        changeData["mt"][i]["Incoterm"] = $("#Incoterm2").val();
                    else
                        changeData["mt"][i]["Incoterm"] = $("#Incoterm").val();
                }
                if (changeData["mt"][i]["IncotermDescp"] != undefined) {
                    if ("F" == tranMode)
                        changeData["mt"][i]["IncotermDescp"] = $("#IncotermDescp2").val();
                    else
                        changeData["mt"][i]["IncotermDescp"] = $("#IncotermDescp").val();
                }
                if (changeData["mt"][i]["LoadingFrom"] != undefined) {
                    if ("F" == tranMode)
                        changeData["mt"][i]["LoadingFrom"] = $("#LoadingFrom2").val();
                    else
                        changeData["mt"][i]["LoadingFrom"] = $("#LoadingFrom").val();
                }
                if (changeData["mt"][i]["LoadingTo"] != undefined) {
                    if ("F" == tranMode)
                        changeData["mt"][i]["LoadingTo"] = $("#LoadingTo2").val();
                    else
                        changeData["mt"][i]["LoadingTo"] = $("#LoadingTo").val();
                }
            } 
        }
    ...
    _handler.beforSave = function () {
        ...
        var TranMode = $("#TranMode").val();
        if ((ContractType === undefined || ContractType === "" || ContractType == null) && (TranMode == "F" || TranMode == "R" || TranMode == "A")) {
            CommonFunc.Notify("", _getLang("L_RQSetup_ContractType", ""), 500, "warning");
            return false;
        }
    ...
    var IncotermOptions = {};
    IncotermOptions.gridUrl = rootPath + LookUpConfig.TermUrl;
    IncotermOptions.registerBtn = $("#IncotermLookup2");
    IncotermOptions.isMutiSel = true;
    IncotermOptions.focusItem = $("#Incoterm2");
    IncotermOptions.columnID = "Cd";
    IncotermOptions.param = "";
    IncotermOptions.gridFunc = function (map) {
    }
    IncotermOptions.responseMethod = function (data) {
        console.log(data);
        var str = "";
    ...
    IncotermOptions.lookUpConfig = LookUpConfig.TermLookup2;
    initLookUp(IncotermOptions);
    ...
    */
    ...
    _handler.beforLoadView = function () {
        ...
        if ($("#TranMode").val() === "F") {
            requires.push("LoadingFrom2");
            requires.push("LoadingTo2");
            $("#NoFclDiv").css("display", "none");
            $("#FclDiv").css("display", "");
        }
    ...
    */
    ...
    MenuBarFuncArr.AddMenu("TerminateBtn", "glyphicon glyphicon-bell", _getLang("L_RQSetup_Termination", ""), function () {
        if (!_handler.topData || isEmpty(_handler.topData[_handler.key])) {
            CommonFunc.Notify("", _handler.lang.tip1, 500, "warning");
            return false;
        }
        ...
        if (_handler.topData["Status"] != "D") {
            CommonFunc.Notify("", _getLang("L_QTSetup_unClosed", ""), 500, "warning");
            return false;
        }
        if (_handler.topData["Status"] === "V") {
            CommonFunc.Notify("", _getLang("L_QTSetup_HasDiscd", ""), 500, "warning");
            return false;
        }

        var truthBeTold = window.confirm(_getLang("L_qtBase_Script_174", ""));
        if (!truthBeTold) {
            return;
        }
        ...
        $.ajax({
            async: true,
            url: rootPath + "RQManage/TerminateRQ",
            type: 'POST',
            dataType: "json",
            data: {
                "UId": _handler.topData["UId"]
    ...
    $("#FreightTerm").change(function () {
        ...
        if ("P" == freightTerm) {
            $("#OutIn").val("O");
        } else if ("C" == freightTerm) {
            $("#OutIn").val("I");
        }
    });

    $("#FreightTermSelect").change(function () {
        var checkParam = $('#FreightTermSelect').find('option:selected');
        var OutInCheck = [];
        for (var i = 0; i < checkParam.length; i++) {
            var sel = checkParam[i].value;
            if ("P" == sel) {
                OutInCheck.push("O");
            } else if ("C" == sel) {
                OutInCheck.push("I");
        ...
        $("#OutInSelect").selectpicker("val", OutInCheck);
    });
    ...
    $("#TranMode").change(function () {
        ...
        //    $("#PodNm")[0].value = "";
        //}, 500);
        appendSelectOption($("#ContractType"), fcnOptions);
        var tranMode = $("#TranMode").val();
        if ("A" == tranMode)
            appendSelectOption($("#ContractType"), acnOptions);
        if ("R" == tranMode)
        ...
        if ("F" == tranMode) {
            ...
            $("#NoFclDiv").css("display", "");
            $("#FclDiv").css("display", "none");
        }

        _handler.beforLoadView();
    });

    $("#LoadingFrom").blur(function () {
        $("#LoadingFrom2").val($("#LoadingFrom").val());
    })
    $("#LoadingFrom2").blur(function () {
        $("#LoadingFrom").val($("#LoadingFrom2").val());
    })
    $("#LoadingTo").blur(function () {
        $("#LoadingTo2").val($("#LoadingTo").val());
    })
    $("#LoadingTo2").blur(function () {
        $("#LoadingTo").val($("#LoadingTo2").val());
    })
});
...
function getSelectOptions() {
    $.ajax({
        ...
        success: function (data) {
            ...
            appendSelectOption($("#LoadingFrom2"), pkOptions);
            appendSelectOption($("#LoadingTo2"), pkOptions);
            appendSelectOption($("#ContractType"), fcnOptions);
            if (_handler.topData) {
                ...
                $("#LoadingFrom2").val(_handler.topData["LoadingFrom"]);
                $("#LoadingTo2").val(_handler.topData["LoadingTo"]);

                var tranMode = _handler.topData["TranMode"];
                $("#TranMode").val(tranMode);
                if ("A" == tranMode)
                    appendSelectOption($("#ContractType"), acnOptions);
                if ("R" == tranMode)
                    appendSelectOption($("#ContractType"), rcnOptions);
