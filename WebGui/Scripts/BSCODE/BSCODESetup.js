...
jQuery(document).ready(function ($) {
    ...
    var colModel = [
    	    ...
    	    { name: 'CdType', title: '@Resources.Locale.L_BSCODESetup_Cd', index: 'CdType', width: 100, sorttype: 'string', editoptions: { size: 4, maxlength: 4 }, editable: false },
    	    { name: 'CdDescp', title: '@Resources.Locale.L_MailGroupSetup_Name', index: 'CdDescp', width: 180, sorttype: 'string', editoptions: { size: 300, maxlength: 300 }, editable: true }
    ];

    var colModel2 = [
            { name: 'Cmp', editable: false, hidden: true, width: 50, order: 0 },
            { name: 'Cd', title: 'BSCODE', index: 'Cd', sorttype: 'string', editoptions: { size: 20, maxlength: 20 }, hidden: false },
	        { name: 'CdDescp', title: '@Resources.Locale.L_MailGroupSetup_Name', index: 'CdDescp', width: 250, sorttype: 'string', editoptions: { size: 300, maxlength: 300 }, editable: true },
            { name: 'CdType', title: '@Resources.Locale.L_BSCODESetup_Cd', index: 'CdType', sorttype: 'string', hidden: true },
            { name: 'OrderBy', title: '@Resources.Locale.L_TKQuery_OrderBy', index: 'OrderBy', sorttype: 'string', width: 70, editoptions: { size: 5, maxlength: 5 }, hidden: false, editable: true },
            { name: 'ApCd', title: 'SCAC', index: 'ApCd', sorttype: 'string', width: 70, hidden: false, editoptions: { size: 50, maxlength: 50 }, editable: true },
            { name: 'ArCd', title: '@Resources.Locale.L_BSCSSetup_Remark', index: 'ArCd', sorttype: 'string', width: 120, hidden: false, editoptions: { size: 24, maxlength: 24 }, editable: true }
    ];
    ...
    new genGrid(
    	$grid,
    	{
    	    datatype: "json",
    	    loadonce: true,
    	    url: rootPath + "BSCODE/CodeKindRequiry",
    	    colModel: colModel,
    	    caption: "BSCODE Kind",
    	    height: gridHeight,
    	    refresh: true,
    	    ...
    	    gridFunc: function (map) {
    	        ...
    	        if (cdType != null && cdType != "") {
    	            ...
    	            $.ajax({
    	                async: true,
    	                url: rootPath + "BSCODE/BscodeRequiry",
    	                type: 'POST',
    	                data: {
    	                    CdType: cdType,
    	                    page: 1,
    ...
    _dm.addDs("BsCodeGrid", [], ["Cd", "CdType", "Cmp"], $BsCodegrid[0]);
    new genGrid(
    	$BsCodegrid,
    	{
    	    data: [],
    ...
    MenuBarFuncArr.MBSave = function (dtd) {
        ...
        $.ajax({
            async: true,
            url: rootPath + "BSCODE/BsCodeUpdate",
            type: 'POST',
