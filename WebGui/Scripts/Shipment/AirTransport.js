$(function () {
    ...
    MenuBarFuncArr.AddMenu("IportBtn", "glyphicon", _getLang("L_SendCargoRequest", "发送货况请求"), function () {
        CommonFunc.ToogleLoading(true);
        $.ajax({
            async: true,
            url: rootPath + "TKBL/SendProlinkTrace",
            type: 'POST',
            dataType: "json",
            data: {
                "uid": $("#UId").val()
    ...
    MenuBarFuncArr.Enabled(["MBEdoc"]);
    MenuBarFuncArr.Enabled(["IportBtn"]);
    getSelectOptions();
    if (!isEmpty(_uid)) {
