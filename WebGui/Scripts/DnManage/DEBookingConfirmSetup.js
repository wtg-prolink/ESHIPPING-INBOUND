...
$(function () {
    ...
    _handler.saveData = function (dtd) {
        ...
        var uid = $("#UId").val();
        var shipmentid = $("#ShipmentId").val();
        ajaxHttpSaveBar(dtd, _handler.saveUrl, { "changedData": encodeURIComponent(JSON.stringify(changeData)), "UId": uid, ShipmentId: shipmentid, autoReturnData: false, "TranType": "D", "IsConfirm": "Y" },
            function (result) {
                //_topData = keyData["mt"];
                if (result.message) {
    ...
    function _setMyReadonlys() {
        var readonlys = ["ShipmentId", "CombineInfo", "Marks", "Instruction", "Goods", "DnEtd", "CostCenter",
         "PpolCd", "PpolName", "PporCd", "PporName", "PpodCd", "PpodName", "PdestCd", "PdestName", "TradeTerm", "TradetermDescp",
         "PaytermCd", "PaytermNm", "BlType", "FrtTerm", "PkgNum", "PkgUnit", "PkgUnitDesc", "ProductDate",
         "ScacCd", "IncotermCd", "Qty", "Qtyu", "Pgw", "Gwu", "Pcbm", "ProfileCd","Lgoods", "BlRmk"
        ];
    ...
    _handler.intiGrid("SubGrid2", $SubGrid2, {
        colModel: colModel2, caption: '@Resources.Locale.L_DEBookingConfirmSetup_Script_126', delKey: ["UId", "PartyType"],
        onAddRowFunc: function (rowid) {
