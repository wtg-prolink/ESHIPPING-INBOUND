...
jQuery(document).ready(function ($) {
    ...
    $("#NOTICEDEL").click(function () {
        if (!confirm("@Resources.Locale.L_initNotice_CleanData")) {
            return;
        }
        StatusBarArr.nowStatus("@Resources.Locale.L_initNotice_Cleaning");
...
function checkId(id) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data) {
                CommonFunc.Notify("", "@Resources.Locale.L_initNotice_NotifyCdExi", 500, "success");
                $("#Id").val('');
            }
        }
    });
