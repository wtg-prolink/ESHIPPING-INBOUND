...
{
    public class RQManageController : BaseController
    {
        //
        // GET: /RQManage/
        ...
        {
            ViewBag.pmsList = GetBtnPms("RQ010");
            SetSchema("RQSetup");
            if (uid == null)
            {
                uid = id;
        ...
        {
            string condition = GetCreateDateCondition("SMRQM", GetRQCondition1());

            string table = @"(SELECT *,(CASE WHEN OUT_IN='O' THEN 'OutBound' WHEN OUT_IN='I' THEN 'INBound' WHEN OUT_IN='O;I' THEN 'OutBound,INBound' ELSE OUT_IN END) OUT_IN_DESC FROM SMRQM)SMRQM";
             
            return GetBootstrapData(table, condition);
        }

        public string GetRQCondition1()
        {
            switch (UPri)
            {
                case "G":
                    return GetBaseGroup();
                default:
                    return GetBaseGroup() + string.Format(" AND RLOCATION={0}", SQLUtils.QuotedStr(CompanyId));
            }
        }
        ...
        public ActionResult SaveRQSetupData()
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(u_id))
                {
                    rfq_no = OperationUtils.GetValueAsString("SELECT TOP 1 RFQ_NO FROM SMRQM WHERE U_ID=" + SQLUtils.QuotedStr(u_id), Prolink.Web.WebContext.GetInstance().GetConnection());
                }
            }
            catch { }
            foreach (var item in dict)
            {
                if (item.Key == "mt")
                {
                    ...
                    for (int i = 0; i < list.Count; i++)
                    {
                        ...
                        if (ei.getNameSet().Contains("RFQ_NO"))
            ...
            if (mixList.Count > 0)
            {
                try
                {
                    if (!string.IsNullOrEmpty(u_id))
                    {
                        mixList.Add(string.Format("UPDATE SMRQM SET STATUS='B' WHERE STATUS='C' AND U_ID={0} AND RFQ_TO>={1}", SQLUtils.QuotedStr(u_id), SQLUtils.QuotedStr(DateTime.Now.ToString("yyyy-MM-dd"))));
                        mixList.Add(string.Format("UPDATE SMRQM SET STATUS='C' WHERE STATUS='B' AND U_ID={0} AND RFQ_TO<{1}", SQLUtils.QuotedStr(u_id), SQLUtils.QuotedStr(DateTime.Now.ToString("yyyy-MM-dd"))));
                    }
            ...
            for (int i = 0; i < lcs.Count; i++ )
            {
                ...
                if(dt.Rows.Count > 0)
                {
                    foreach (DataRow item in dt.Rows)
                    {
                        ...
                        ei.Put("STN", Station);
                        ei.Put("DEP", Dep);
                        ei.PutDate("CT_DATE", Prolink.Math.GetValueAsDateTime(item["EFFECT_FROM"]));
                        ei.PutDate("EFFECT_TO", Prolink.Math.GetValueAsDateTime(item["EFFECT_TO"]));
                        ei.Put("APPROVE_TO", "A");
                        ml.Add(ei);
                    }
                }
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult InitiatedCheck() {
            //string uid = Request["UId"];
            //DataTable dt = OperationUtils.GetDataTable(string.Format("SELECT QUOT_TYPE,RFQ_NO,TRAN_MODE FROM SMQTM WHERE U_ID={0}", SQLUtils.QuotedStr(uid)), null, Prolink.Web.WebContext.GetInstance().GetConnection());
            //if (dt.Rows.Count <= 0)
            //    return Json(new { message = @Resources.Locale.L_QTManage_Controllers_395, flag = false });
            //string tranmode = Prolink.Math.GetValueAsString(dt.Rows[0]["TRAN_MODE"]);
            ////string ApproveTo = Prolink.Math.GetValueAsString(dt.Rows[0]["APPROVE_TO"]);
            //string quotmode = "QUOT_" + tranmode;
            //string approvesql = string.Format("SELECT * FROM APPROVE_FLOW_D WHERE APPROVE_CODE='{0}' ORDER BY APPROVE_LEVEL", quotmode);
            ...
            //{
            //    quotmode = "QUOT_CON";
            //    approvesql = string.Format("select * from APPROVE_FLOW_D where APPROVE_CODE='{0}'  ORDER BY APPROVE_LEVEL", quotmode);
            //    approvedt = OperationUtils.GetDataTable(approvesql, null, Prolink.Web.WebContext.GetInstance().GetConnection());
            //}
            //if (!(approvedt != null && approvedt.Rows.Count > 0))
            //    return Json(new { message = @Resources.Locale.L_QTManage_Controllers_tip1, flag = false });
            //string returnMsg = string.Empty, NextDnType = string.Empty,noticeto = string.Empty,notice_mail = string.Empty;
            ...
            //};
            //string sql = string.Format("SELECT COUNT(REF_NO) FROM APPROVE_RECORD WHERE REF_NO='{0}'",uid);
            //int count = OperationUtils.GetValueAsInt(sql, Prolink.Web.WebContext.GetInstance().GetConnection());


            //string qtsql= string.Format("SELECT * FROM  SMQTM WHERE  U_ID='{0}'", uid);
            //DataTable qtdt = OperationUtils.GetDataTable(sql, null,Prolink.Web.WebContext.GetInstance().GetConnection());
            ...
            //if (count == 0)
            //{
            //    QuotApprove.CreateQuotApprove(ml, approvedt, userinfo, uid);
            //    returnMsg = @Resources.Locale.L_ActCheckSetup_Scripts_34;
            //    approveto = userinfo.Dep;
            //}
            //else
            //{
            //    //string 
            //    //return Json(new { message = @Resources.Locale.L_DNFlowManage_Views_340, flag = false });
            //    //签核
            //    if (qtdt != null && qtdt.Rows.Count > 0)
            //    {
            //        approveto = Prolink.Math.GetValueAsString(qtdt.Rows[0]["APPROVE_TO"]);
            //        string sql1 = string.Format("SELECT * FROM APPROVE_RECORD WHERE REF_NO='{0}' ORDER BY VOID_LOOP,APPROVE_LEVEL ASC", uid);
            //        DataTable adt = OperationUtils.GetDataTable(sql1, null, Prolink.Web.WebContext.GetInstance().GetConnection());
            //        for (int i = 0; i < adt.Rows.Count; i++)
            //        {
            //            DataRow dr = adt.Rows[i];
            //            string status = Prolink.Math.GetValueAsString(dr["STATUS"]);
            //            if (status == "1") continue;
            //            if (i != 0)
            //            {
            //                QuotApprove.Approve(ml, adt, userinfo, i);
            //            }
            //        }
            //    }
            //}
            //try {
            //    var result = OperationUtils.ExecuteUpdate(ml,Prolink.Web.WebContext.GetInstance().GetConnection());
            //    //APPROVE_TO
            //}
            //catch (Exception e)
            //{
            //    return Json(new { message = @Resources.Locale.L_QTManage_Controllers_tip2, flag = false });
            //}
            //return Json(new { message = returnMsg == "" ? @Resources.Locale.L_RQManage_Controllers_456 : returnMsg, flag = true });
            string returnMsg = "";
            ...
            else
            {
                ...
                for (int i = 0; i < uids.Length; i++)
                {
                    ...
                    returnMsg += QuotApprove.ApproveQuotItem(uid, userinfo) + "\n";
                }
            }
        ...
        public ActionResult ApproveBackQuot()
        {
            ...
            string message =QuotApprove.QuotApproveBack(uid, backremark, userinfo);
        ...
        public ActionResult SubmitQT()
        {
            ...
            EditInstruct ei = new EditInstruct("SMQTM", EditInstruct.UPDATE_OPERATION);
            ei.PutKey("U_ID", uid);
            ei.PutKey("QUOT_TYPE", key_quot_type);
            ...
            if (tran_mode.Equals("C") || tran_mode.Equals("B") || tran_mode.Equals("X") || tran_mode.Equals("O"))
            {
                ei.Put("APPROVE_TO","A" );
            }
            ml.Add(ei);
        ...
        public ActionResult TerminateRQ()
        {
            string uid = Request["UId"];
            DataTable dt = OperationUtils.GetDataTable(string.Format("SELECT STATUS,RFQ_NO FROM SMRQM WHERE U_ID={0}", SQLUtils.QuotedStr(uid)), null, Prolink.Web.WebContext.GetInstance().GetConnection());
            if (dt.Rows.Count <= 0)
                return Json(new { message = @Resources.Locale.L_QTManage_Controllers_395, flag = false });
            string status = Prolink.Math.GetValueAsString(dt.Rows[0]["STATUS"]);
            string rfq_no = Prolink.Math.GetValueAsString(dt.Rows[0]["RFQ_NO"]);
            if (!"D".Equals(status))
            {
                return Json(new { message = @Resources.Locale.L_QTSetup_unClosed, flag = false });
            }
            if ("T".Equals(status))
            {
                return Json(new { message = @Resources.Locale.L_QTSetup_Terminated, flag = false });
            }
            ...
            OperationUtils.ExecuteUpdate(ml, Prolink.Web.WebContext.GetInstance().GetConnection());
            return Json(new { message = @Resources.Locale.L_QTSetup_TerminateSuccess, flag = true });
        }

        public ActionResult TerminateQT()
        {
            string uid = Request["UId"];
            DataTable dt = OperationUtils.GetDataTable(string.Format("SELECT TRAN_MODE,QUOT_TYPE,RFQ_NO,LSP_CD FROM SMQTM WHERE U_ID={0}", SQLUtils.QuotedStr(uid)), null, Prolink.Web.WebContext.GetInstance().GetConnection());
            if (dt.Rows.Count <= 0)
                return Json(new { message = @Resources.Locale.L_QTManage_Controllers_395, flag = false });
            string quot_type = Prolink.Math.GetValueAsString(dt.Rows[0]["QUOT_TYPE"]);
            string rfq_no = Prolink.Math.GetValueAsString(dt.Rows[0]["RFQ_NO"]);
            string lspCd = Prolink.Math.GetValueAsString(dt.Rows[0]["LSP_CD"]);

            if ("T".Equals(quot_type))
            {
                return Json(new { message = @Resources.Locale.L_QTSetup_Terminated, flag = false });
            }
