...
{
    public class BSLIGHTController : BaseController
    {
        #region View
        public ActionResult QueryView()
        {
            ViewBag.MenuBar = false;
            ViewBag.pmsList = GetBtnPms("LI010");
            return View();
        }
        ...
        public ActionResult SetupView(string id = null, string uid = null)
        {
            ...
            ViewBag.Uid = id;
            ViewBag.pmsList = GetBtnPms("LI010");
            string edtsel = CommonHelp.getBscodeForColModel("EDT", GetDataPmsCondition("C"));
                
            ViewBag.TranSel = CommonHelp.getBscodeForColModel("TNT", GetDataPmsCondition("C"));

            edtsel = ":;" + edtsel;
            ViewBag.EdtSel = edtsel.Replace("'", ""); ;
        ...
        public ActionResult QueryData()
        {
            ...
            condition = GetCreateDateCondition("BSLIGHTM", condition);
            return GetBootstrapData("BSLIGHTM", condition);
        }
        #endregion
        ...
        public ActionResult GetDetail()
        {
            ...
            string sql = string.Format("SELECT * FROM BSLIGHTM WHERE U_ID={0}", SQLUtils.QuotedStr(u_id));
            DataTable mainDt = OperationUtils.GetDataTable(sql, null, Prolink.Web.WebContext.GetInstance().GetConnection());

            sql = string.Format("SELECT * FROM BSLIGHTD WHERE U_FID={0}", SQLUtils.QuotedStr(u_id));
            DataTable subDt = OperationUtils.GetDataTable(sql, null, Prolink.Web.WebContext.GetInstance().GetConnection());

            Dictionary<string, object> data = new Dictionary<string, object>();
            data["main"] = ModelFactory.ToTableJson(mainDt, "BslightmModel");
            data["sub"] = ModelFactory.ToTableJson(subDt, "BslightdModel");
        ...
        public ActionResult UpdateData()
        {
            ...
            if (!string.IsNullOrEmpty(CustCd))
                CustCd = HttpUtility.UrlDecode(CustCd);
            ...
            foreach (var item in dict)
            {
                if (item.Key == "mt")
                {
                    ...
                    MixedList list = ModelFactory.JsonToEditMixedList(objList, "BslightmModel");
                    for (int i = 0; i < list.Count; i++)
                    {
                        EditInstruct ei = (EditInstruct)list[i];
                        if (ei.OperationType == EditInstruct.INSERT_OPERATION)
                        {
                            string[,] unikey = new string[1, 2] { { "CUST_CD", ei.Get("CUST_CD") } };

                            if (chkKeyIsExist("BSLIGHTM", unikey) == true)
                            {
                ...
                else if (item.Key == "sub")
                {
                    ...
                    MixedList list = ModelFactory.JsonToEditMixedList(objList, "BslightdModel");
                    for (int i = 0; i < list.Count; i++)
                    {
                        EditInstruct ei = (EditInstruct)list[i];
                        ...
                        EditInstruct mei = new EditInstruct("BSLIGHTM", EditInstruct.UPDATE_OPERATION);
                        mei.PutKey("U_ID", u_id);
                        mei.PutDate("MODIFY_DATE", DateTime.Now);
                        mei.Put("MODIFY_BY", UserId);
                        mixList.Add(mei);
                        mixList.Add(ei);
                    }
                }
            ...
            sql = string.Format("SELECT * FROM BSLIGHTM WHERE U_ID={0}", SQLUtils.QuotedStr(u_id));
            DataTable mainDt = OperationUtils.GetDataTable(sql, null, Prolink.Web.WebContext.GetInstance().GetConnection());
            ...
            if (!SchemasCache.ContainsKey(name))
            {
                ...
                switch (name)
                {
                    case "BSLIGHT":
                        ...
                        sql = "SELECT * FROM BSLIGHTM WHERE 1=0";
                        break;
                }
