...
namespace WebGui.Controllers
{
    public class CostCenterController : BaseController
    {
        ...
        #region View
        public ActionResult CostCenterQueryView()
        {
            ViewBag.MenuBar = false;
            ViewBag.pmsList = GetBtnPms("CostCenter");
            return View();
        }

        public ActionResult CostCenterSetupView(string id = null, string uid = null)
        {
        ...
        public ActionResult QueryData()
        {
            ...
            condition = GetCreateDateCondition("SMCC", condition);
            return GetBootstrapData("SMCC", condition);
        }
        #endregion
        ...
        public ActionResult GetDetail()
        {
            ...
            string sql = string.Format("SELECT * FROM SMCC WHERE U_ID={0}", SQLUtils.QuotedStr(u_id));
            DataTable mainDt = OperationUtils.GetDataTable(sql, null, Prolink.Web.WebContext.GetInstance().GetConnection());

            Dictionary<string, object> data = new Dictionary<string, object>();
            data["main"] = ModelFactory.ToTableJson(mainDt, "SMCCModel");
        ...
        public ActionResult UpdateData()
        {
            ...
            foreach (var item in dict)
            {
                if (item.Key == "mt")
                {
                    ...
                    MixedList list = ModelFactory.JsonToEditMixedList(objList, "SMCCModel");
                    for (int i = 0; i < list.Count; i++)
                    {
                        ...
                        if (ei.OperationType == EditInstruct.INSERT_OPERATION)
                        {
                            ...
                            //if (chk == false)
                            //{
                            //    return Json(new { message = "资料重複，保存失敗" });
                            //}
            ...
            string sql = string.Format("SELECT * FROM SMCC WHERE U_ID={0}", SQLUtils.QuotedStr(UId));
            DataTable mainDt = OperationUtils.GetDataTable(sql, null, Prolink.Web.WebContext.GetInstance().GetConnection());
            ...
            if (!SchemasCache.ContainsKey(name))
            {
                ...
                switch (name)
                {
                    case "CostCenter":
                        ...
                        sql = "SELECT * FROM SMCC WHERE 1=0";
                        break;
                }
