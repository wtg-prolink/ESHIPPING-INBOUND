...
{
    public class IPQ02DataGetter : Prolink.WebReport.Business.IDataGetter
    {
        ...
        public DataSet GetDataSet(NameValueCollection nameValues)
        {
            ...
            */
            ...
            if (uId != "")
            {
                condition = " IU_FID=" + SQLUtils.QuotedStr(uId);
            }
            
            DataSet ds = new DataSet();
            int recordsCount = 0, pageIndex = 0, pageSize = 0;

            DataTable dt = ModelFactory.InquiryData("V_IPQ02.*", "V_IPQ02", condition, nameValues, ref recordsCount, ref pageIndex, ref pageSize);
            ...
            string sql = "SELECT CNTR_NO, SEAL_NO1, CNTY_TYPE,DN_NO FROM SMRV WHERE SHIPMENT_ID IN (" + relFilter + ") ";
