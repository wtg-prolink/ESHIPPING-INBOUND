...
{
    public class FCL01DataGetter : Prolink.WebReport.Business.IDataGetter
    {
        ...
        public DataSet GetDataSet(NameValueCollection nameValues)
        {
            ...
            */
            ...
            string condition = "";
            DataTable dt = ModelFactory.InquiryData("V_FCL01.*", "V_FCL01", condition, nameValues, ref recordsCount, ref pageIndex, ref pageSize);
            dt.TableName = "FCL01";
            ...
            string sql = "SELECT SMDN.TRAN_TYPE,SMDNP.*,SMDN.SHIPMENT_ID, SM.* FROM SMDN LEFT JOIN SMDNP ON SMDN.DN_NO=SMDNP.DN_NO LEFT JOIN Smcuft SM ON SMDN.DN_NO=SM.DN_NO WHERE ISNULL(SMDN.SHIPMENT_ID,'') <> '' AND SMDN.SHIPMENT_ID IN (" + relFilter + ") ";
            DataTable SMDNP = GetDataTable("SMDNP", sql);
            sql="SELECT * FROM SMDN WHERE SMDN.SHIPMENT_ID IN (" + relFilter + ") ";
            DataTable SMDN = GetDataTable("SMDN", sql);
