...
namespace Business
{
    public static class DateTimeUtils
    {
        ...
        public static DateTime? ParseToDateTimeForNullValue(string dateStr)
        {
            DateTime dt = ParseToDateTimeByFormat(dateStr);
            if (dt == DateTime.MinValue) return null;
            return dt;
        }

        public static DateTime ParseToDateTimeByFormat(string dateStr)
        {
            List<string> formarts = new List<string>() { "yyyyMMddHHmmss", "yyyyMMddHHmm", "yyyyMMdd" };
            return ParseToDateTime(dateStr, formarts);
        }

        public static DateTime ParseToDateTime(string dateStr)
        {
            List<string> formarts = new List<string>() { "yyyy-MM-dd", "yyyyMMdd", "yyyy/MM/dd", "yyyyMMddHHmmss", "yyyyMMddHHmm", "yyyyMMdd" };
            return ParseToDateTime(dateStr, formarts);
        }

        public static DateTime ParseToDateTime(string dateStr, List<string> formarts)
        {
            if (string.IsNullOrEmpty(dateStr)) return DateTime.MinValue;
            DateTime time;
            foreach (var f in formarts)
            {
                if (DateTime.TryParseExact(dateStr, f, System.Globalization.CultureInfo.CurrentCulture.DateTimeFormat,
